...
| `GET /api/wrapped` 🔒 | `?jahr=JJJJ&g=` — der Jahresrückblick einer Runde. **`?g=` ist Pflicht** (Etappe 11): bis dahin rechnete er über die ganze Instanz und zeigte die Namen aller Menschen. Der Schnitt läuft über `gruppe_id`, wo die Tabelle eine führt, und sonst über die Mitgliedschaft — `reports` trägt keine Gruppe (0033). Meldungen zählen dabei erst **ab dem Beitritt**, sonst schleppte ein Dezember-Beitritt elf fremde Monate herein; ein Ausgetretener verschwindet dagegen rückwirkend, während Buchungen, Strafen und Termine ihn behalten — die tragen eine echte `gruppe_id`. Hängt an `statistik_an`, das der Wirt überspringt (Entscheidung 25). Die Antwort trägt `runde` (Tafel, Rad, Notruf, Termine) und `verein` (Kasse, Hausordnung) — jedes davon `null`, wenn die Runde diese Seite nicht führt, und das ist etwas anderes als leere Zahlen. `ich` und `mein_verein` fehlen, wer nicht Mitglied ist (der Wirt in einer fremden Runde). Ein abgeschlossenes Jahr darf die Edge einen Tag halten; `?g=` gehört zum Cache-Schlüssel |
